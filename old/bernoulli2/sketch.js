...
function draw() {
	...
	if(isDrawing == false){
		...
		while( counter < Nperstep) {
		    //		    		var b = new Particle(0,random(0.2*height,0.8*height),particle_radius);
		    		var b = new Particle(0,0.5*height,particle_radius);
		    //		    var b = new Particle(0,counter/(Nperstep-1)*height,particle_radius);
		particles.push(b);
		counter = counter + 1;
		}
	}
