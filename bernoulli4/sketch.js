...
function draw() {
	...
	if(isDrawing == false){
		...
		textSize(20);
		if(checkClick == false){
			text("Click to add objects!",0.29*width,0.1*height);
		}
		else{
		...
		//		}
		//		counter = 0;
		//		while( counter < Nperstep) {
		    //		    		var b = new Particle(0,0.5*height,particle_radius);
		//		particles.push(b);
		//		counter = counter + 1;
		//		}
		if (firsttime == true) {
		    if ( counter < Nbeginning) {
			var b = new Particle(0,random(0.35*height,0.65*height),particle_radius);
			particles.push(b);
			counter = counter + 1;
		    } else {
               	firsttime = false;
		...
		//		if( (spaceispressed == true) && (firsttime == false) )
		//		{
		//			var b = new Particle(0,random(0.35*height,0.65*height),particle_radius);
		//			particles.push(b);
		//		}
		...
		if(checkClick2 == false){
			text("Draw a pipe! Make it smaller in the middle than on the sides.",0.02*width,0.1*height);
